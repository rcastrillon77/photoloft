...
function setupStripeElements() {
    const stripe = Stripe("pk_live_51Pc8eHHPk1zi7F68Lfo7LHLTmpxCNsSidfCzjFELM9Ajum07WIMljcsbU9L1R2Tejvue1BaZ0xuDwcpiXjwMgrdq00eUxlyH9D");
...
function renderSelectedOptions() {
    ...
    .map(title => {
        ...
        id: data.id,
        ...data,
        count: String(parseInt(data.count || "0", 10) + 1)
        };
    })
    .filter(Boolean);
...
function updatePurposeHiddenField() {
    ...
    const selected = selectedActivities
      .map(title => {
        const data = bookingTypes[title];
        if (!data) return null;
        return { id: data.id, ...data, count: String(parseInt(data.count || "0", 10) + 1)};
      })
      .filter(Boolean);
