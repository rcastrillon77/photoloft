document.addEventListener("DOMContentLoaded", async () => {
    console.log("✅ booking.js loaded again");
  try {
...
// ** UI ENHANCERS ** //
  ...
  function updateMaxAvailableButton() {
    ...
    // Add all values from slider range (including half steps)
    ...
    // Sort the options and filter for ones <= available
  ...
  function highlightSelectedDate() {
    ...
    // Remove any existing custom highlight
    document.querySelectorAll('.flatpickr-day.selected-date').forEach(el => {
      el.classList.remove('selected-date');
    });
  
    // Add highlight class to the correct day
...
function disableUnavailableDates(instance) {
  ...
  requestAnimationFrame(() => {
    document.querySelectorAll('.flatpickr-day').forEach(day => {
      ...
      if (dayStart.getMonth() !== currentMonth || dayStart.getFullYear() !== currentYear) {
        return;
      }
...
function initCalendar() {
  ...
  if (!document.getElementById('date-picker').value) {
    ...
    window.bookingGlobals.booking_date = new Date(); // also update global
  }
